...
But what makes the CLT so profound is its universality. It doesn't matter what the probability distribution of your initial random variable is. You could be rolling a standard six-sided die, flipping a biased coin, or measuring something with a bizarre, skewed distribution that looks nothing like a bell curve. As long as you perform the experiment repeatedly and independently, the result is always the same.

This is where the concept of **IID (Independent and Identically Distributed)** random variables comes in.
...
When these conditions are met, the CLT states that if you take a large number of these variables, $X_1, X_2, \dots, X_m$, the distribution of their **sum** ($S_m = X_1 + \dots + X_m$) or their **average** ($\bar{X}_m = S_m/m$) will converge to a perfect bell curve.
